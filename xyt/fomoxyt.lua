...
------------------------------------------------------------------------------------
--http://fomoxyt.com    XYT auction for RNTP  v2.0
...
Send = function()
	...
	if tx.addr==Fomoaddress  and curaddr~=_G.Config.owner then
		...
		local tpx = 5.01
		local addtimes = 600
		local zq=173400 --2*24*60*60+addtimes  87010
		...
		if curaddr~=pstr[1] and blkts<=blasts and blkts>blasts-zq and (top==10000 or (tx.money>=top+addone and tx.money<=top*tpx)) then
			...
			estrs=estrs.." BiJingPai.com XYT too Small"
			end
			if tx.money>top*tpx then
			estrs=estrs.." BiJingPai.com XYT too Big"
			end
			error(estrs)
		end
	end	
...
Main()
--[[-------test--www.fomoxyt.com--xyt---2019-08-16 12:00-----------------
contracts={"f0110000"
...
,"f03698cf585d0000" --设置竞拍初始的截止时间戳 开始为一天前  8月18日12:10 f03698cf585d0000  8月16日12:10->  f036982c565d0000 只能由owner直接指定
,"f033000077585974624c77773165564241397974556e6b5058516d4e6a6f47563348754652731027000000000000402a565d00000000" --设首拍卖不能Fomoaddress数量须10000下可随意高
